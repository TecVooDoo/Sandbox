...
name: uc-query
description: |-
  Reads the full Ultimate Character Controller setup on a GameObject.
...
```bash
unity-mcp-cli run-tool uc-query --input '{
...
> ```bash
> unity-mcp-cli run-tool uc-query --input-file args.json
> ```
>
> Or pipe via stdin (recommended):
> ```bash
> unity-mcp-cli run-tool uc-query --input-file - <<'EOF'
> {"param": "value"}
> EOF
> ```
