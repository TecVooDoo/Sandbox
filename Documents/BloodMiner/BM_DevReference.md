# Blood Miner -- Dev Reference

**Purpose:** Architecture, coding standards, and AI rules for Blood Miner. Read on demand -- primary doc is `BM_Status.md`.
...
| Namespace | Purpose |
|-----------|---------|
| `BM.Core` | Currency, rank system, game state, save/load |
| `BM.Harvest` | Tap input, scythe harvest, soul extraction, combo system |
...
| `BM.VFX` | Gore effects, soul wisps, blood spray, body parts, screen shake |
...
|
+-- Scripts/
|   +-- Core/           -- BloodManager, RankSystem, GameState, SaveManager
...
```
TAP INPUT --> TapHarvester --> BloodManager (add currency)
                |                    |
                v                    v
...
```
TapHarvester
    +--- OnSoulHarvested ---> BloodManager (add), VFX (gore burst + wisp), Audio (crunch + chime)
...
Warehouse
    ...
    +--- OnCollected ---> BloodManager (add spendable currency)
```
...
1. **Primary doc:** `BM_Status.md` -- read first, always.
2. **Working directory:** `E:\Unity\Sandbox`
3. **SM root:** `Assets/_Sandbox/_BM/`
