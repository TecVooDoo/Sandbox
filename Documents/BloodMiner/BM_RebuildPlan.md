# Blood Miner -- Sprint 2 Rebuild Plan
...
|----------------|----------|
| `BM.Core.GameEvent` / `GameEvent<T>` / listeners | Event architecture is sound |
| `BloodManager` | Currency tracking is unchanged |
| `SaveManager` (Easy Save 3) | Save system works, will adapt data shape |
| `NumberPop` (BM.VFX) | Floating numbers on chop are still wanted |
| `ComboSystem` (BM.Harvest) | Combo-while-actively-clicking is in v2.0 |
...
| `Ghoul` : RowWorker | Player avatar. Player clicks to chop, combo tracked. |
| `ChopMinion` : RowWorker | Auto-chops. Spawned when outlet is purchased. |
| `LeftoversGauge` | Per-row fill progress. Events on full / emptied. |
...
1. Create new scene `BM_Shaft.unity` (keep `BM_ShallowGraves.unity` as-is for reference; don't delete yet)
2. Create `BM.Shaft` and `BM.Gatherer` namespace folders
...
1. Move `BM_ShallowGraves.unity` to `Assets/_Sandbox/_BM/Scenes/Archive/`
2. Move `MineLevel.cs`, `Elevator.cs`, `Warehouse.cs`, `ZoneInitializer.cs`, `RankSystem.cs` to `Assets/_Sandbox/_BM/Scripts/Archive/` (or delete if nothing's reused)
