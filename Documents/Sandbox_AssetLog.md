...
### MCP Controllability Evaluated -- Sandbox Session 3 (Mar 20, 2026)
...
| A* Pathfinding Project Pro 5.4.6 | ENTRY-164 | **High** | Source-based UPM package at `Packages/com.arongranberg.astar`. Namespace: `Pathfinding`. Assembly: `AstarPathfindingProject`. Key components: `AstarPath` (singleton manager, holds graphs, `Scan()`, `UpdateGraphs()`), `GridGraph` (width, depth, nodeSize, center, maxSlope, maxStepHeight, collision, erosion, neighbours), `RecastGraph` (characterRadius, walkableHeight, walkableClimb, cellSize, contourMaxError, useTiles), `NavMeshGraph` (sourceMesh, offset, rotation, scale), `Seeker` (graphMask, traversableTags, tagCostMultipliers, modifiers), `AIPath`/`AILerp`/`RichAI` (maxSpeed, rotationSpeed, endReachedDistance, slowdownDistance, pickNextWaypointDist, constrainInsideGraph), `AIDestinationSetter`, `GraphUpdateScene` (polygon area, penalty/walkability/tag modification), `RVOAgent` (local avoidance: radius, agentTimeHorizon, priority, layer). Modifiers: `SimpleSmoothModifier`, `FunnelModifier`, `RaycastModifier`. Tag system: 32 tags, per-tag cost multipliers. | **Candidate -- 6 tools.** Graph setup (GridGraph has 15+ fields, RecastGraph has 10+) and AI agent config (AIPath has 15+ tunable fields) are the main MCP wins -- each collapses 10+ `component-modify` calls into one. Runtime graph updates (penalty, walkability, tag changes within bounds) are a natural MCP action. Proposed: `astar-query` (AstarPath state: graph list, node counts, scan status + AI agent state on a GO), `astar-configure-grid` (GridGraph: dimensions, nodeSize, center, rotation, maxSlope, maxStepHeight, erosion, collision, neighbours), `astar-configure-recast` (RecastGraph: characterRadius, walkableHeight, cellSize, maxSlope, tiling), `astar-configure-agent` (AIPath/AILerp/RichAI: maxSpeed, rotationSpeed, endReachedDistance, slowdownDistance, pickNextWaypointDist, orientation), `astar-scan` (trigger Scan() on all or specific graph, runtime UpdateGraphs with bounds + penalty/walkability/tag params), `astar-configure-seeker` (Seeker: graphMask, traversableTags, tagCostMultipliers, modifier settings). Assembly: `AstarPathfindingProject`. Same publisher as ALINE. |
| Dialogue System for Unity 2.2.66 | ENTRY-214 | **High** | Source-based, namespace `PixelCrushers.DialogueSystem`, no asmdef (Assembly-CSharp). ~518 runtime scripts. Static facade `DialogueManager` with string-based API. Key API surfaces: **Conversations** (`StartConversation(title, actor, conversant, initialEntryID)`, `StopConversation()`, `ConversationHasValidEntry()`, `isConversationActive`, `currentActor/currentConversant`), **Lua variables** (`Lua.Run(code)`, `Lua.IsTrue(condition)` -- full Lua execution for all Variable[], Actor[], Item[] tables), **Quests** (`QuestLog.SetQuestState(name, state)`, `CurrentQuestState()`, `SetQuestEntryState()`, `GetQuestDescription()`, `GetQuestNames()`, states: unassigned/active/success/failure), **Barks** (`Bark(title, speaker, listener)`, `BarkString(text, speaker)`), **Alerts** (`ShowAlert(msg, duration)`, `HideAlert()`), **Sequences** (`PlaySequence(sequence, speaker, listener)` -- Camera/Delay/Animator/Audio/Show/Hide commands), **Actor config** (`ChangeActorName()`, `SetPortrait()`), **Database** (`AddDatabase()`, `RemoveDatabase()`, `masterDatabase` with actors/conversations/items/variables/locations lists), **Localization** (`SetLanguage(lang)`). Data model: DialogueDatabase (SO) -> Conversation -> DialogueEntry -> Link. DialogueEntry has conditionsString (Lua), userScript (Lua), Sequence, onExecute (UnityEvent). | **Candidate -- 6 tools.** The `DialogueManager` static facade maps cleanly to MCP. Conversation start/stop, quest state management, and Lua variable access are the highest-value targets -- each is a single static call but requires knowing exact method signatures and string keys. A `ds-query` tool that reads database metadata (conversation list, actor list, variable list, quest states) is extremely valuable for discoverability. Proposed: `ds-query` (list conversations/actors/variables/quests from database + active conversation state), `ds-conversation` (start/stop conversation by title, with actor/conversant GO names, initial entry ID), `ds-quest` (get/set quest state and entry states by quest name), `ds-variable` (get/set Lua variables by name -- wraps `Lua.Run("Variable['name']")` and `Lua.Run("Variable['name'] = value")`), `ds-bark` (trigger bark from conversation or raw text on a speaker GO), `ds-lua` (execute arbitrary Lua code and return result -- power tool for advanced users). No asmdef -- `#if HAS_DIALOGUE_SYSTEM` guard. Detection: `PixelCrushers.DialogueSystem.DialogueManager, Assembly-CSharp`. |
| GOAP v3 3.1.1 (CrashKonijn) | -- | **Medium** | Source-based UPM package at `Packages/com.crashkonijn.goap`. 5 assemblies: `CrashKonijn.Agent.Core`, `.Runtime`, `CrashKonijn.Goap.Core`, `.Resolver` (Burst), `.Runtime`. Code-first fluent builder for agent types: `AgentTypeBuilder` -> `CapabilityBuilder` -> `ActionBuilder`/`GoalBuilder`/`SensorBuilder`. Key runtime components: `GoapActionProvider` (`RequestGoal<T>()`, `StopAction()`, `ClearGoal()`, `Enable<T>()`/`Disable<T>()`), `GoapBehaviour` (manager, `GetAgentType(id)`), `IAgentType` (`GetActions()`, `GetGoals()`, `Agents` collection). Planning: backward search from goal conditions to current world state via resolver job (Burst). World state via `IWorldKey`/`ITargetKey` marker interfaces + sensors. `AgentState` enum: NoAction, StartingAction, PerformingAction, MovingToTarget, MovingWhilePerformingAction. | **Deferred -- not a candidate now.** Config is code-compile-only (fluent builders produce immutable configs). No runtime sensor/action cost mutation API. World state (`ILocalWorldData`) is opaque -- no direct key/value read access. The main MCP value (goal setting, plan inspection, action enable/disable) is achievable via `script-execute` with 2-3 static calls. Dedicated tools would mostly wrap `RequestGoal<T>()` and `GetActions()/GetGoals()` -- marginal benefit over script-execute. Revisit if a project heavily adopts GOAP. |
| Breeze 1.0.2 (Breeze Assets) | -- | **Medium-High** | Source-based at `Assets/Breeze/`, namespace `Breeze.Core`, no asmdef (Assembly-CSharp). Main component: `BreezeSystem` MonoBehaviour with 50+ public serializable fields covering health (StartingHealth, MaximumHealth, RegenerateHealth/Frequency/Amount), behavior (AIBehaviour enum: Enemy/Companion, AIConfidence: Brave/Neutral/Coward, WanderType: Patrol/Waypoint/Stationary), movement (WalkSpeed, RunSpeed, RotationSpeed, UseRootMotion), combat (AttackDistance, MinAttackDelay, MaxAttackDelay, UseBlockingSystem, HitReactionPossibility), detection (DetectionType: LineOfSight/Trigger, DetectionDistance, DetectionAngle, DetectionLayers), IK (UseHandIK, UseAimIK, AimSmoothAmount), faction system (CurrentAIFaction, AIFactionsList dict), companion (CFollowingDistance, CSprintFollowDistance, AttackBehaviour: Aggressive/Passive). Methods: `TakeDamage()`, `SetDestination()`, `PlayAnimation()`, `BreezeEquip()`/`BreezeHolster()`, `NotifySound()`. Integrations: UCC, Invector, NeoFPS, A* Pathfinder. | **Deferred -- possible candidate (4 tools) if adopted.** Massive public field surface makes it excellent for MCP. A `breeze-query` + `breeze-configure` + `breeze-combat` + `breeze-detection` tool set would collapse many `component-modify` calls. But the asset is currently Deferred in AssetLog (pending hands-on eval). No project actively uses it. Revisit when a project adopts Breeze. Detection: `Breeze.Core.BreezeSystem, Assembly-CSharp`. |
| Follow and Protect Agent 1.0 | -- | **Low** | Source-based at `Assets/FollowProtectAgent/`, namespace `FollowProtectAgent`. ML-Agents-based companion AI using pre-trained ONNX model for inference-only deployment. Key component: `FollowProtectAgent` (extends `Unity.MLAgents.Agent`) with followDistance, detectionRange, attackRange, damage, fireRate fields. `Health` component: TakeDamage(), Heal(), IsDead(), GetHealthPercentage(), ResetHealth(). Events: OnEnemyKilled, OnPlayerProtected, OnAgentDamaged. | **Not a candidate.** Core behavior is a neural network black box -- cannot command agent to attack/follow/hold via MCP. `OnActionReceived()` is called internally by ML-Agents, not scriptable. Health and spawn methods are single `script-execute` calls. Limited to parameter tweaking (followDistance, damage) via `component-modify`. No dedicated tools needed. |
| Procedural Dialogue Addon 1.0.5 | ENTRY-215 | **Low** | Source-based, namespace `PixelCrushers.DialogueSystem.ProceduralDialogue`, 26 scripts (~2000 LOC). Grammar-driven text transformation addon for Dialogue System. Key components: `ProceduralDialogueAddon` (singleton, `ParseText(text, actor)`, `GetMood/SetMood` Lua functions, hooks into OnConversationLineEarly/OnBarkLine), `ProceduralDialogueActor` (per-actor config: Mood, DialectTable, GrammarSets), `ProceduralTextParser` (template expansion: `{{word}}`, `{{word1/word2}}` random, `{{var name}}`, `{{lua code}}`, `{{mood = value}}`), `EnglishProceduralTextParser` (verb conjugation, article agreement, pluralization). Grammar SOs: `ProceduralGrammar` (word swap tables), `GenderedGrammar` (he/she/they), `PirateGrammar`, `HighMedievalGrammar`, `LeetspeakGrammar`. Dialect via TextTable assets. | **Not a candidate.** The addon is a text transformation pipeline that hooks into Dialogue System events automatically. Configuration is per-actor (`ProceduralDialogueActor.Mood` property, dialect/grammar SO references) which is easily handled via `component-modify`. The `ParseText()` method is a single static call via `script-execute`. No multi-field setup complexity that warrants dedicated tools. Mood get/set is already Lua-exposed (`GetMood(actorName)`, `SetMood(actorName, mood)`) so the `ds-lua` tool covers it. |
| Ultimate Character Controller 3.3.3p1 | ENTRY-165 | **High** | Source-based, 485 scripts (58K LOC). Namespace: `Opsive.UltimateCharacterController.*`. Assemblies: `Opsive.UltimateCharacterController.Runtime`, `.Editor`. UPM package: `com.opsive.ultimatecharactercontroller`. Depends on `com.opsive.shared`. Key components: `UltimateCharacterLocomotion` (movement, gravity, states, speed sets), 39+ built-in `Ability` subclasses (Jump, Fall, Slide, Interact, Drive, Ragdoll, etc.), `ItemCollection` (inventory), `CharacterItem` + `ItemAction` (MeleeAction, ShootableAction, ThrowableAction, MagicAction), `Attribute` system (Health, Stamina, Mana), SO-based `State` system (hot-swappable), `CameraController` with `ViewType` perspective switching. Runtime API: `Move()`, `Sprint_Set()`, `TryStartAbility()`, `StopAbility()`, `EquipItem()`, `OnDamage()`, `State_Activate()`. | **Candidate -- 5 tools.** Querying full character state (locomotion + abilities + items + attributes + active state) via `component-get` requires 10+ calls; a single `uc-query` collapses this. Ability enable/disable and speed tuning are multi-field operations. Same publisher as BD Pro -- integration designed. Proposed: `uc-query` (full character state: active abilities, equipped items, speed sets, attributes, current state), `uc-configure-locomotion` (movement type, gravity, mass, speed sets), `uc-ability-control` (enable/disable/start/stop abilities by type name), `uc-configure-attribute` (set health/stamina/mana values, min/max), `uc-item-control` (equip/unequip, set loadout index, query inventory). Camera perspective switching and damage application are single `script-execute` calls -- not worth dedicated tools. Note: UCC demo content not fully imported (only ImportStatus marker in Assets/Opsive/); runtime API available via UPM package. |

### MCP Controllability Evaluated -- Sandbox Session 4 (Mar 20, 2026)

ECS/DOTS packages evaluated for SpaceSucks project support.

| Asset | ENTRY | Rating | Key API Pattern | Notes |
|-------|-------|--------|-----------------|-------|
| Unity Entities 1.4.5 | -- | **Medium-High** | `Unity.Entities` namespace. `World.All` to enumerate worlds, `World.DefaultGameObjectInjectionWorld` for default. `EntityManager.GetComponentTypes(entity)` for component list. `EntityManager.CreateEntityQuery(ComponentType[])` for dynamic queries. `CreateEntity`/`DestroyEntity` for lifecycle. Generic `GetComponentData<T>`/`SetComponentData<T>` require reflection for runtime types. Play-mode only (no World in edit mode). | **Candidate -- 5 tools built.** `ecs-query-worlds`, `ecs-query-entities`, `ecs-inspect-entity`, `ecs-modify-entity`, `ecs-create-destroy`. Reflection layer for runtime generic component access. Foundation tools that Unity Physics runtime components also use. |
| Unity Physics 1.4.5 | -- | **Medium** | Two surfaces: **Authoring** (edit-time MonoBehaviours): `PhysicsBodyAuthoring` (mass, damping, gravity, kinematic, initial velocity), `PhysicsStepAuthoring` (gravity, substeps, solver iterations, broadphase), `PhysicsShapeAuthoring` (collision filter, material friction/restitution). **Runtime** (ECS): `PhysicsVelocity`, `PhysicsMass`, `PhysicsDamping`, `PhysicsGravityFactor` -- writable IComponentData. Collider geometry is immutable blob (NOT modifiable at runtime). Namespace: `Unity.Physics.Authoring` (authoring), `Unity.Physics` (runtime). | **Candidate -- 4 tools built.** `uphys-query`, `uphys-configure-body`, `uphys-configure-step`, `uphys-configure-shape`. Authoring components are the edit-time MCP surface; runtime velocity/mass changes go through Entities tools. |
| Entities Graphics 1.4.18 | -- | **Low** | Automatic baking of MeshRenderer/MeshFilter to ECS rendering components. `RenderFilterSettings` (layer, shadow mode) is the only useful tweak. Material/mesh arrays are immutable shared components. No meaningful authoring surface beyond standard Unity components. | **Not a candidate.** Almost entirely automatic infrastructure. 1-2 useful operations handled by `script-execute` or Entities query tools. |
