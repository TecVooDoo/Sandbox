...
**Primary output:** `Sandbox_AssetLog.md` -- 353 asset evaluations as of Session 82.
...
| 350 | Sound Shapes (TelePresent Games) | **Approved, Recommended, Audio** | © 2025 Martin Hansen. 3D zone-based ambient audio. 17 scripts (~3000 LOC), 12 demo prefabs (Lake/Tunnel/Bush/Pole/Cloud/etc.), 1 demo scene. 3 zone modes (Shape/Mesh/MultiEmitter), Tag/Object tracking, raycast occlusion w/ lowpass cutoff blend (7000Hz↔22000Hz), DualAudio inside/outside swap, BVH spatial accel for mesh zones. Pairs with Master Audio (default install — different layer: MA is global, SS is per-scene 3D). Strong fit: HOK (Acheron audio), AQS (forest), HnR (stealth — occlusion is gameplay), SetDesign. **MCP: Medium-High** — 5 tools proposed for new `ss` group (`ss-create-zone`, `ss-add-points`, `ss-configure-occlusion`, `ss-configure-tracking`, `ss-query`). |
| 351 | UI Shader Effects: Edge Effects (RVFX) | **Approved, Recommended, UI** | UGUI shaders + UIToolkit ShaderGraphs in one pack — 7 UGUI shaders + 7 UIToolkit ShaderGraphs (additive + multiplicative variants of EdgeEffect / Electric / base UIEffect). 14+ pre-built EdgeGlow prefabs for UIToolkit. Demo scenes for both UI systems. AI-generated icon library + Mask + Pattern textures. 73 MB. **Strong fit:** HOK / FS / HnR / VNPC / M3 / BM (active UI feedback). Pairs with Synty InterfaceDarkFantasyHUD (ENTRY-332), Modular 3D Text (ENTRY-341), Damage Numbers Pro, Feel. **MCP: Low** — pure shader/material content. |
| 352 | UIMotionFX Tool (RVFX, "UI Effects") | **Approved, Recommended, UI** | Sister product to ENTRY-351, different abstraction. Two sub-tools: (1) **UIMotionFX Painter** editor brush tool that paints flow + glow maps onto images (`BrushSettings`/`ProcessFlowmap`/`ProcessGlowmap`/`ImageMotionEffect`/`MotionFXExporter`); (2) **Compute Shader UI Particles** (BloomGlow/Geometry/Snow runtime presets — GPU-driven, frees CPU). UGUI + UIToolkit branches. 35 prefabs, 11 ImageMotionEffect samples, AI-generated texture library (6 themes), dedicated `UIMotionFX_Painter.unity` editor scene. **341 MB** — prunable (drop unused texture themes + UGUI-or-UIToolkit branch). **Strong fit:** HOK / VNPC / M3 (custom-authored UI motion). Compute particles are GPU-bound, mobile-viable on SM 5.0+. **MCP: Low** — painter is mouse-driven editor flow. |
| 353 | VFX Graph Mega Pack Vol 2 v1.3 (Gabriel Aguiar Productions) | **Approved, Recommended, VFX** | **URP unitypackage extracted this session.** `com.unity.visualeffectgraph 17.3.0` resolves cleanly on Unity 6000.3.10f1 (the `6.3` in filename was internal pack version, not Unity-target). **42 VFX Graph assets** across 7 categories: HitsImpacts (9), Lightning (4 — w/ GradientNoise/SimpleNoise/Voronoi shader variants), LootDrops (5 in own folders), MagicOrbs (10), MuzzleFlashes (4+), Portals, SummonCreatures, WeaponFX (paired w/ Axe/Dagger/Hammer/Shield textures). **106 prefabs**, **7 demo scenes** (one per category), **10+ ShaderGraphs** (Distortion / Galaxy / Glow / 4 Lightning variants / 2 MeshEffect PBR), full Animations / Materials / Textures / Settings folders. **553 MB on disk** post-extraction. Complementary to Polygon Arsenal (Shuriken/CPU) + Vefects (blood). **Strongest fits in current catalog:** HOK (MagicOrbs underworld + Portals + WeaponFX scythe + Lightning), HnR (HitsImpacts + Lightning + SummonCreatures + WeaponFX Reap), FS (HitsImpacts + WeaponFX + MuzzleFlashes if applicable), AQS (MagicOrbs + Portals + LootDrops), M3, SetDesign. Skip BM (mobile GPU budget). At standalone migration, prune to needed categories — likely halves install. **MCP: Low-Medium** — VFX Graph editing is node-based; runtime `VisualEffect.SetFloat/SetVector/SetTexture` via `gameobject-component-modify`. |

**Ultimate Preview 2 upgrade (no new ENTRY):** Voxel Labs released Ultimate Preview 2 as a major-version successor to Ultimate Preview Window Pro v1.3.4 (originally evaluated as ENTRY-343). User updated in-place this session — same publisher, same install path (`Assets/Voxel Labs/Ultimate Preview/`), same Asset Store slug, same DLL namespace, same labels (**Default, QoL**). v2 changes are file-system observable: new `Render Pipeline/` folder with HDRP-via-unitypackage support, structured `UltimatePreviewSettingsData` SO with camera orbit/FOV/lighting config, "Reset to HDRP/URP Default" quick-apply buttons. Tracked as an **ENTRY-343 Addendum** in `Sandbox_AssetLog.md` rather than a new ENTRY (same product). [NewProjectSetup_Brief.md](NewProjectSetup_Brief.md) and [Sandbox_FreshInstall.md](Sandbox_FreshInstall.md) install rows updated to reflect v2 branding + HDRP install note.
...
- **Retained:** VFX Graph Mega Pack Vol 2 (Gabriel Aguiar, ENTRY-353) — **URP unitypackage extracted this session**, ENTRY-353 upgraded from provisional to confirmed. 553 MB on disk; 42 VFX Graphs + 106 prefabs + 7 demo scenes across HitsImpacts/Lightning/LootDrops/MagicOrbs/Muzzles/Portals/SummonCreatures/WeaponFX.
- **Retained (upgraded):** Ultimate Preview 2 (Voxel Labs) — v1.3.4 → v2 in-place upgrade.
- **Removed post-eval:** Arcade Vehicle Physics, Simulation Game Creator, Sound Shapes (TelePresent Games), UI Shader Effects: Edge Effects (RVFX, ENTRY-351), UIMotionFX Tool (RVFX, ENTRY-352). All five eval-captured; re-import when target use case (kart-racer / sim project / atmospheric audio integration / UI VFX integration) is in scope.
