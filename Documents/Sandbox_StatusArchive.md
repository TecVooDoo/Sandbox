...
- Raccoon_Weapon_Test created from PA Player prefab. Belly weapon system wired. LMB routing still blocked (Attack1 melee intercepts).

*MCP Connection Standardization:*
- Created `MCP_ConnectionBrief.md`. Audited all 8 projects. Added Blender MCP to all projects. Port registry documented. MCP plugin updated to v0.62.0.

*Global Permissions Cleanup:* Cleaned `~/.claude/settings.json`, added all 8 projects to `additionalDirectories`.

*Package Management:* Switched UniTask, Image Loader, Unity Theme from local file to OpenUPM.

*New Project Setup Brief:* Created `NewProjectSetup_Brief.md`.

*Status Doc Archive System:* Created StatusArchive files for 4 projects. Trimmed status docs.
